...
const EditTournamentModal = ({ isOpen, onClose, onTournamentUpdated, tournament, user }) => {
  ...
  // Pre-fill form when tournament data is available
  useEffect(() => {
    if (tournament) {
      setFormData({
        name: tournament.name || '',
        description: tournament.description || '',
        date: tournament.date ? tournament.date.slice(0, 16) : '', // Format for datetime-local
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiRequest(`${API_ENDPOINTS.TOURNAMENTS}/${tournament.tournament_id}`, {
        method: 'PUT',
      ...
      if (response.success) {
        ...
        setMessage(`Failed to update tournament: ${response.error || 'Unknown error'}`)
      }
    } catch (error) {
      ...
      <div className="card" style={{ maxWidth: '500px', width: '100%', maxHeight: '90vh', overflow: 'auto' }}>
        <div className="card-header">
          <div className="flex justify-between items-center">
            <h2 className="card-title text-xl">Edit Tournament</h2>
            <button
              onClick={onClose}
              style={{
                background: 'none',
          ...
          <p className="card-description">Update tournament details</p>
        </div>
